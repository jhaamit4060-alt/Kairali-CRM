...
export async function GET(req: NextRequest) {
    ...
    try {
        ...
        if (!gasRes.ok) {
            console.error("[get-part1] upstream returned status", gasRes.status)
        ...
        try {
            ...
            console.error("[get-part1] upstream returned non-JSON body")
            return NextResponse.json({ status: "error", message: "Upstream error" }, { status: 502 })
        }

        if (j.status !== "success" || !Array.isArray(j.data)) {
            console.error("[get-part1] unexpected upstream payload shape")
            return NextResponse.json({ status: "error", message: "Upstream error" }, { status: 502 })
        }
        ...
        if (err?.name === "AbortError") {
            console.error("[get-part1] upstream request timed out")
            return NextResponse.json({ status: "error", message: "Upstream timed out" }, { status: 504 })
        }

        console.error("[get-part1] upstream request failed")
