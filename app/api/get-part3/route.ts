...
export async function GET(req: NextRequest) {
    ...
    try {
        const gasRes = await fetch(`${GAS_URL}?action=getAll&part=3`, {
            cache: "no-store",
            redirect: "follow",
        ...
        if (!gasRes.ok) {
            console.error("[get-part3] upstream returned status", gasRes.status)
        ...
        try {
            ...
            console.error("[get-part3] upstream returned non-JSON body")
            return NextResponse.json({ status: "error", message: "Upstream error" }, { status: 502 })
        }

        if (j.status !== "success" || !Array.isArray(j.data)) {
            console.error("[get-part3] unexpected upstream payload shape")
            return NextResponse.json({ status: "error", message: "Upstream error" }, { status: 502 })
        }
        ...
        if (err?.name === "AbortError") {
            console.error("[get-part3] upstream request timed out")
            return NextResponse.json({ status: "error", message: "Upstream timed out" }, { status: 504 })
        }

        console.error("[get-part3] upstream request failed")
